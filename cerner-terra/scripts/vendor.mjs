...
// Message locales, least to most specific — later files win.
const MESSAGE_LOCALE_FALLBACKS = ["en", "en-CA"];
...
// Collect Terra's own translations for the locale we default to.
//
// The shim's message map used to be hand-written, which meant it silently
// covered only the components we had happened to render: Terra's
// DemographicsBanner asks for ten message ids and the map had one, so the
// banner rendered the literal string `Terra.demographicsBanner.dateOfBirth`
// on screen. Generating it from each package's own translations removes the
// whole class of gap — and it is authoritative, e.g. terra-date-picker's
// en-CA file is where `dateFormat: YYYY-MM-DD` comes from.
{
  const messages = {};
  let files = 0;
  for (const pkg of [...allPackages, ...SCSS_PACKAGES]) {
    const pkgDir = findPackage(pkg) ?? (FRAMEWORK_PACKAGES[pkg] ? fetchFromNpm(pkg, FRAMEWORK_PACKAGES[pkg]) : null);
    if (!pkgDir) continue;
    // Fall back through the locale chain the way Terra's own loader does.
    for (const tag of MESSAGE_LOCALE_FALLBACKS) {
      const file = join(pkgDir, "translations", `${tag}.json`);
      if (!existsSync(file)) continue;
      Object.assign(messages, JSON.parse(readFileSync(file, "utf8")));
      files += 1;
    }
  }
  const sorted = Object.keys(messages).sort();
  writeFileSync(
    join(pkgRoot, "src", "runtime", "terra-messages.ts"),
    `/**\n * Generated by scripts/vendor.mjs from the vendored packages' own\n` +
      ` * translations (${MESSAGE_LOCALE_FALLBACKS.join(" then ")}). Do not edit.\n */\n` +
      `export const TERRA_MESSAGES: Record<string, string> = {\n` +
      sorted.map((k) => `  ${JSON.stringify(k)}: ${JSON.stringify(messages[k])},`).join("\n") +
      `\n};\n`,
  );
  console.log(`  merged ${sorted.length} messages from ${files} translation files`);
}
