...
## Checking it against Terra

Terra's own docs site is the reference. Its component pages render on a plain
`terra-base` document, so the numbers are directly comparable:

```
https://engineering.cerner.com/terra-ui/components/cerner-terra-core-docs/form-input/about
```

Measured there and here, `Input` and `Field` agree to the sub-pixel — 16px
text, 29.9844px tall, 3.99994px padding, `1px solid #dedfe0`, 3px radius,
`border-box`; label 14px/700/#1c1f21; root 14px Helvetica Neue on `#fff` with
`dir=ltr`. Their site sets **no** `--terra-*` custom properties at the root,
so both sides render the components' own fallback values — there is no theme
to match.

If a difference shows up, it is almost certainly in the *shell*, not the
components. Three pre-fork approximation layers used to cause exactly that:
a `#f4f4f4` page background painted over terra-base's `#fff`, a hand-rolled
demographics banner standing in for Terra's real one, and a blanket
`h1..h6 { color: #333 }` that repainted the banner's white name. The
approximation layer in the player's `terra-tokens.scss` now stands down
whenever `<TerraBase>` marks the document, so it only styles the Fluent/MOIS
path it was written for.

What legitimately differs is the page around the form: their site is
`terra-dev-site`, an application shell with nav, sidebar and tabs. We render
a bare form under a demographics banner.
