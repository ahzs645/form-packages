...
const MultiTargetChoiceField = ({
  ...
  const applyForces = (d) => {
    options.forEach((option) => {
      ...
      const anyOn = triggers.some((code) => {
        ...
        return opt ? optionChecked(d, opt) : false
      })
      if (anyOn) d[force.fieldId] = toCoding(force.value, undefined, (option.target || {}).system)
    })
  }
  ...
  const toggle = (option, next) => {
    ...
    setFormData(produce((draftFd) => {
      ...
      const d = draftFd.field.data
      const target = option.target || {}
      if (target.mode === "arrayMember") {
        const currentArr = asArray(d[target.fieldId])
        const has = currentArr.some((entry) => codeOf(entry) === target.value)
        if (next && !has) {
          d[target.fieldId] = [...currentArr, toCoding(target.value, option.display, target.system)]
        } else if (!next && has) {
          d[target.fieldId] = currentArr.filter((entry) => codeOf(entry) !== target.value)
        }
      } else {
        d[target.fieldId] = next ? toCoding(target.value, option.display, target.system) : null
      }
      applyForces(d)
      if (effectiveFieldId) {
        d[effectiveFieldId] = options.filter((entry) => optionChecked(d, entry)).map((entry) => entry.code)
      }
    }))
  }
